    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.utility-categories.update", [$utilityCategory->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('utility_type') ? 'is-invalid' : '' }}" name="utility_type" id="utility_type" required>
                    ...
                    @foreach(App\Models\UtilityCategory::UTILITY_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('utility_type', $utilityCategory->utility_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('utility_name') ? 'is-invalid' : '' }}" type="text" name="utility_name" id="utility_name" value="{{ old('utility_name', $utilityCategory->utility_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('utility_website') ? 'is-invalid' : '' }}" type="text" name="utility_website" id="utility_website" value="{{ old('utility_website', $utilityCategory->utility_website) }}">
