...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.utilityAccount.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.utility-accounts.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('utility_name') ? 'is-invalid' : '' }}" name="utility_name_id" id="utility_name_id" required>
                    @foreach($utility_names as $id => $utility_name)
                        <option value="{{ $id }}" {{ old('utility_name_id') == $id ? 'selected' : '' }}>{{ $utility_name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('account_number') ? 'is-invalid' : '' }}" type="text" name="account_number" id="account_number" value="{{ old('account_number', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('account_phone') ? 'is-invalid' : '' }}" type="text" name="account_phone" id="account_phone" value="{{ old('account_phone', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('account_status') ? 'is-invalid' : '' }}" name="account_status" id="account_status" required>
                    ...
                    @foreach(App\Models\UtilityAccount::ACCOUNT_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('account_status', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
