    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.invoices.update", [$invoice->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('utility_type') ? 'is-invalid' : '' }}" name="utility_type_id" id="utility_type_id" required>
                    @foreach($utility_types as $id => $utility_type)
                        <option value="{{ $id }}" {{ (old('utility_type_id') ? old('utility_type_id') : $invoice->utility_type->id ?? '') == $id ? 'selected' : '' }}>{{ $utility_type }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('utility_companyname') ? 'is-invalid' : '' }}" name="utility_companyname_id" id="utility_companyname_id" required>
                    @foreach($utility_companynames as $id => $utility_companyname)
                        <option value="{{ $id }}" {{ (old('utility_companyname_id') ? old('utility_companyname_id') : $invoice->utility_companyname->id ?? '') == $id ? 'selected' : '' }}>{{ $utility_companyname }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('account_number') ? 'is-invalid' : '' }}" name="account_number_id" id="account_number_id" required>
                    @foreach($account_numbers as $id => $account_number)
                        <option value="{{ $id }}" {{ (old('account_number_id') ? old('account_number_id') : $invoice->account_number->id ?? '') == $id ? 'selected' : '' }}>{{ $account_number }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('property_type') ? 'is-invalid' : '' }}" name="property_type_id" id="property_type_id" required>
                    @foreach($property_types as $id => $property_type)
                        <option value="{{ $id }}" {{ (old('property_type_id') ? old('property_type_id') : $invoice->property_type->id ?? '') == $id ? 'selected' : '' }}>{{ $property_type }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_due') ? 'is-invalid' : '' }}" type="text" name="date_due" id="date_due" value="{{ old('date_due', $invoice->date_due) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('payment_status') ? 'is-invalid' : '' }}" name="payment_status" id="payment_status" required>
                    ...
                    @foreach(App\Models\Invoice::PAYMENT_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('payment_status', $invoice->payment_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
